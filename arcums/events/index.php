...
if(isset($_SESSION['dj_logged_in'])){
	...
	if($_GET['all']=='1'){
		$result =  mysql_query("SELECT title, start, end, location, details, id FROM events ORDER BY start ASC, end DESC")
		or die("Databse Error");
	}
	else{
		$result = mysql_query("SELECT title, start, end, location, details, id FROM events WHERE end > Now() or (start>=Now() and end='0000-00-00 00:00:00') ORDER BY start ASC, end DESC")
