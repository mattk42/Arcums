...
if($user_info['permissions']>=2){
	if(isset($_GET['UPC'])){
		...
		$pxml = aws_signed_request("com", array("Operation"=>"ItemSearch","SearchIndex"=>"Music","Keywords"=>$upc,"ItemPage"=>$page,"ResponseGroup"=>"Large"), $public_key, $private_key);
		...
		//echo "SELECT ID FROM albums WHERE UPC='" . $upc . "' AND Category_ID='" . $cat . "' AND Category_Number='" . $cat_num . "'";
				...
				if(sizeof($pxml->Items->Item->Tracks->Disc)>0){
					foreach ($pxml->Items->Item->Tracks->Disc as $disc) {
						...
						if(!$insert){echo "<b>Disc:</b> " . $disc_number . "<br>";}
						foreach ($disc->Track as $track){
							$track_number = $track->attributes()->Number;
							if(!$insert){echo "<b>" . $track_number . ".</b> " . $track . "<br>";}
							else{
								$len = "00:00";
								mysql_query("INSERT INTO tracks(album_id,position,title,cd_number,length) VALUES('" . $album_id . "','" . mysql_real_escape_string($track_number) . "','" . mysql_real_escape_string($track) . "','" . mysql_real_escape_string($disc_number) . "','" . mysql_real_escape_string($len) . "')")or die(mysql_error());
		...
		if(!$insert){
					...
					$res = mysql_query("SELECT DISTINCT(name), id FROM category_lookup") or die(mysql_error());
			...
			echo "<meta HTTP-EQUIV='REFRESH' content='0; url=http://www.wupx.com/arcums/catalog/view.php?id=" . $album_id . "'>";
		}
