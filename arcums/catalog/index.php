...
//$query is the query for the albums themselvs, c_query is the query for the total count
		$query = "SELECT DISTINCT albums.* FROM albums  WHERE albums.artist LIKE '%" . $artist . "%' AND albums.title LIKE '%" . $title . "%' AND albums.category_id LIKE '" . $cat_id .  "' ORDER BY albums." . $order_s . " " . $dir . " LIMIT ". $start ."," . $page_limit;

		$c_query = "SELECT DISTINCT albums.* FROM albums  WHERE albums.artist LIKE '%" . $artist . "%' AND albums.title LIKE '%" . $title . "%' AND albums.category_id LIKE '" . $cat_id ."'" ;
			$list_tracks = false;
		if($track!=""){
			$query = "SELECT DISTINCT albums.* , tracks.position, tracks.title FROM tracks INNER JOIN albums ON albums.id=tracks.album_id WHERE albums.artist LIKE '%" . $artist . "%' AND albums.title LIKE '%" . $title . "%' AND tracks.title LIKE '%" . $track . "%' AND albums.category_id LIKE '" . $cat_id . "'ORDER BY albums." . $order_s . " ". $dir ." LIMIT ". $start ."," . $page_limit;
		$c_query = "SELECT DISTINCT albums.* , tracks.position, tracks.title FROM tracks INNER JOIN albums ON albums.id=tracks.album_id WHERE albums.artist LIKE '%" . $artist . "%' AND albums.title LIKE '%" . $title . "%' AND tracks.title LIKE '%" . $track . "%' AND albums.category_id LIKE '" . $cat_id ."'";
			$list_tracks = true;
		}
		$album_query = mysql_query($query) or die(mysql_error());
		$total = mysql_num_rows(mysql_query($c_query));	
		...
		while($row = mysql_fetch_array($album_query)){
			...
			$cat_query = mysql_query("SELECT prefix FROM category_lookup WHERE id='" . $row['category_id'] . "'");
			echo "<td>";
